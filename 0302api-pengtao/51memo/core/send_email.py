...
def send_email_attach(body, attachment, to_addr):
    ...
    with open(attachment, 'rb') as f:
        # MIMEBase表示附件的对象
        mime = MIMEBase('pdf', 'txt', filename=attachment)
        # filename是附件的名字,注意！这个filename并不一定等于# 附件的名字，它是传到邮箱时可以改成其他名字
...
def send_email_all(body, attachment):
    ...
    with open(attachment, 'rb') as f:
        ...
        msg.attach(mime)
    try:
        smtp.sendmail("sublime11@foxmail.com", "\
        sublime11@qq.com", msg.as_string())
        smtp.quit()
    except smtplib.SMTPException as e:
        print(e)
