...
def config(data_file_path):
    ...
    section_name = (os.path.splitext(data_file_path)[0]).split(os.path.sep)[-1]      # 章节名
    data_name = (os.path.splitext(data_file_path)[0]).split(os.path.sep)[-1]         # 数据文件名字
    data_type = os.path.splitext(data_file_path)[1].strip('.')                       # 数据文件类型
    log = mylog.my_logger(base_path + '\\log\\' + 'config_log.log', logging.DEBUG, 'pengtaolog' )
    config_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    config_path = os.path.join(config_path +r'\conf', 'common.ini')    # 配置文件名路径
    try:                                          # 尝试打开配置文件并添加个人配置，若无配置文件这生成
        config = configparser.ConfigParser()
        config.read(config_path)                  # 打开配置文件
        config.add_section(section_name)          # 添加用户个人章节
        items_list = {'data_name':data_name, 'data_type':data_type, 'data_path':data_path}   
        # 配置项
...
def main():
    """主函数调用config（）函数进行调试"""
    # config_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    config(r'C:\Users\Administrator\Desktop\my_code\exercise\homework\51memo\db\pengtao.pkl')
